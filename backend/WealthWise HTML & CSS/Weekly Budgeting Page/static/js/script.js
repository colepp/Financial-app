...
async function getTransactions(){
  ...
  const lastMonthResponse = await fetch(`${api_url}/api/get_last_week_date`);
  ...
  const transactionsResponse = await fetch(`${api_url}/api/get_weekly_transactions`,{
    method: 'POST',
  ...
  transactions.forEach(item => {
    ...
    //Creation of the area under line
    const area = d3.area()
        .x(d => x(d.date))
        .y0(height) 
        .y1(d => y(d.amount))
    ...
    //Appending the path of the area under line
    svg.append("path")
        .datum(actualTransactions)
        .attr("fill", "#b8d9d0")
        .attr("d", area);

    //Appending the path of the line
    svg.append("path")
        .datum(actualTransactions)
        .attr("fill","none")
        .attr("stroke", "#84dcc6")
